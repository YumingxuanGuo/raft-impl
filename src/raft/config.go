...
func (cfg *config) checkTimeout() {
	...
	if !cfg.t.Failed() && time.Since(cfg.start) > 1000*time.Second {
	...
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
			...
			if term == -1 {
				...
				fmt.Printf("%d term: %d, %d xterm: %d", i-1, term, i, xterm)
...
func (cfg *config) end() {
	...
	if cfg.t.Failed() == false {
		...
		fmt.Printf("  ... Passed --")
		fmt.Printf("  time:%4.1f | nPeers: %d | nRPCs:%4d | nBytes:%7d | nCMDs:%4d\n",
